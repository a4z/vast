...
//  * Add `VAST_SDT()` convenience macro.
...
/// @param name The name of the trace point. An invocation `VAST_USDT(foo)` will
///             create a tracepoint that can be accessed as `%sdt_vast:foo` by
///             `perf probe` at al.
